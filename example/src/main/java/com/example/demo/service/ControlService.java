...
public class ControlService {
	...
	public ModelAndView confirmSubService(String hostName, String hostIp, String clickId,Integer transactionId) throws URISyntaxException {
		/*
		 * logger.info("saving user hit details in user hit table IP:" + hostIp +
		 * " User Agent" + hostName); int userHitId =
		 * userHitsService.saveUserHitInfo(hostName, hostIp);
		 * logger.info("user hit id details is: " + userHitId + " IP: " + hostIp +
		 * " User Agent" + hostName); //ad product_id int transactionId =
		 * transactionService.saveTransactionInfo(userHitId,clickId);
		 */
	...
	public String MTService(String msisdn, String trx_id, String session_id, String shortcode, String telco,String message, String price, String service, String subject, String content_type)throws URISyntaxException, UnsupportedEncodingException {
		...
		transactionService.updateBillingTransaction(msisdn, value.getPackPrice(), session_id,"PENDD",arrayOfResponseBody[1]);
		return mt;
	}
