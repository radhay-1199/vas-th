...
public class MainController {
	...
	public ModelAndView invokeService(HttpServletRequest request) throws URISyntaxException {
		String hostName=request.getHeader("User-Agent");
		String hostIp=request.getRemoteAddr();
		logger.info("request received for subscribe from IP:"+hostIp+" and User Agent"+hostName);
		logger.info("Redirecting");
		return controlService.confirmSubService(hostName,hostIp);
	}
	...
	public ResponseEntity<?> DRControl(@RequestParam String shortcode, @RequestParam String telco, @RequestParam String msisdn, @RequestParam String service, @RequestParam(required=false) String subject, @RequestParam String session_id, @RequestParam String trx_id, @RequestParam String status) {
		...
		try {
			if("ALERT;REG;01".equals(subject) && status.equals("RECEIVED")){
				logger.info("inserting into com_subscribed_users_details");
			...
			if("ALERT;REG2;01".equals(subject) && status.equals("RECEIVED")) {
				logger.info("updating charge amount in com_transaction");
				transactionservice.updateBillingTransactionCharge(session_id);
			}
			if("ALERT;REG2;01".equals(subject) && status.equals("REJECTED")) {
				transactionservice.updateBillingStatusAndResponse(session_id);
			}
			if("ALERT;UNREG;01".equals(subject) && status.equals("RECEIVED")) {
			...
			if("ALERT;PUSH;01".equals(subject) && status.equals("RECEIVED")) {
				message=message+Base64.getEncoder().encodeToString(msisdn.getBytes("utf-8"));
				transactionservice.updateBillingTransactionCharge(session_id);
			...
			if("ALERT;PUSH;01".equals(subject) && status.equals("REJECTED")) {
				transactionservice.updateBillingStatusAndResponse(session_id);
			}
		} catch (Exception e) {
